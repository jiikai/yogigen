...
int frontpage_handler(struct mg_connection *conn, void *cbdata)
{
    int ret;
    ...
    mg_printf(conn, bdata(server->html_template), server->btn_txts[rnd], "template", "", "", "template");
    return 200;
error:
    if (ret) {
...
int gen_request_handler(struct mg_connection *conn, void *cbdata)
{
    ...
    printf("%s\n", bdata(gen_str_enc));
    mg_printf(conn, bdata(server->html_template), "Generate\nnew", "div", bdata(gen_str),
        bdata(gen_str_enc), "div");
    bdestroy(gen_str);
...
YogiServer *YogiServer_init()
{
    ...
    FILE *fp = fopen(YOGIGEN_HTML, "r");
    check(fp, YOGISERVER_FILE_ERR, "opening", YOGIGEN_HTML);
    server->html_template = bread((bNread) fread, fp);
    check(fp, YOGISERVER_FILE_ERR, "reading", YOGIGEN_HTML);
    fclose(fp);
    ...
    mg_set_request_handler(server->ctx, FRNTPG_URI, frontpage_handler, server);
...
void YogiServer_close(YogiServer *server)
{
    if (server) {
        ...
        if (server->html_template) {
            bdestroy(server->html_template);
        }
